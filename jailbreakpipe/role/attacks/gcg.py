...
class GCGAttackerConfig(BaseAttackerConfig):
    ...
    adv_string_init: str = field(default="x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x")
...
class GCGAttacker(BaseAttacker):
    ...
    def attack(
        ...
        for _ in tqdm(range(self.num_steps)):
            ...
            with torch.no_grad():
                ...
                if self.filter_ids:
                    sampled_ids = filter_ids(sampled_ids, self.llm.tokenizer)
    ...
    def compute_token_gradient(
        ...
        if self.use_mellowmax:
            ...
            loss = mellowmax(-label_logits, alpha=self.mellowmax_alpha, dim=-1)
        else:
